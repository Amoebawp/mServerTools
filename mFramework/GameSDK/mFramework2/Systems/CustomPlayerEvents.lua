    ...
    if (not event) then
        if (type(method) == 'function') then
            ...
            mFramework2.Debug('CustomPlayer',
              string.expand('registered Event: ${name}', {name = eventType}))
            return true, 'event created'
        end
        mFramework2.Debug('CustomPlayer',
          string.expand('failed to register Event: ${name}', {name = eventType}))
...
local evt_created, evt_msg = mFramework2.CreatePlayerEvent('runScript',
                               function(player, event, sourceId, targetId)
...
if (not evt_created) then
    mFramework2.Debug('CustomPlayer',
      string.expand('register Event failed: runScript > ${res}', {res = evt_msg}))
else
    mFramework2.Debug('CustomPlayer',
      string.expand('register Event Ok: runScript > ${res}', {res = evt_msg}))
end
