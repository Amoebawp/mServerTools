...
function ChatCommand:reply(player, msg)
    ...
    g_gameRules.game:SendTextMessage(0, player._id,
      string.expand(template, {command = self.name, content = msg}))
end
...
function ChatCommand:run(player, command, ...)
    ...
    if type(response) == 'string' then
        if type(self['reply']) == 'function' then self:reply(player, response) end
    end
    return ok, response
end
