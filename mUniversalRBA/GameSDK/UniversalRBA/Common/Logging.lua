if not g_UniversalRBA then return end

local function isDebug()
    local dbg = false
    if (System.GetCVar('log_verbosity') == 3) then
        dbg = true
    elseif (g_UniversalRBA.LOGLEVEL >= 3) then
        dbg = true
    end
    return dbg
end
...
local function init()
    ...
    g_UniversalRBA.Log = function(source, message)
        if not (g_UniversalRBA.LOGLEVEL >= 1) then return end
        return writer('LOG', source, message)
    end

    --- Writes a [Error] level entry to the mFramework log
    g_UniversalRBA.Err = function(source, message)
        if not (g_UniversalRBA.LOGLEVEL >= 1) then return end
        return writer('ERROR', source, message)
    end

    --- Writes a [Warning] level entry to the mFramework log
    g_UniversalRBA.Warn = function(source, message)
        if not (g_UniversalRBA.LOGLEVEL >= 2) then return end
        return writer('WARNING', source, message)
    end
    --- Writes a [Debug] level entry to the mFramework log
    g_UniversalRBA.Debug = function(source, message)
