...
--- @param param any the param to bind
--- @param kind string the params bound type
--- @usage
...
function assert_arg(arg, param, kind)
    if type(param) ~= kind then
        local fn = debug.getinfo(2, 'n')
        local msg = 'Invalid Param in [' .. fn.name .. '()]> ' ..
                      string.format('Argument:%s Type: %q Expected: %q', tostring(arg), type(param),
                        kind)
